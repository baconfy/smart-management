...
export const VoiceSelector = ({ value: valueProp, defaultValue, onValueChange, open: openProp, defaultOpen = false, onOpenChange: onOpenChangeProp, children, ...props }: VoiceSelectorProps) => {
    ...
    const handleOpenChange = useCallback(
        (nextOpen: boolean, eventDetails: Parameters<NonNullable<typeof onOpenChangeProp>>[1]) => {
            setOpen(nextOpen);
...
export const VoiceSelectorAge = ({ className, ...props }: VoiceSelectorAgeProps) => <span className={cn('text-xs text-muted-foreground tabular-nums', className)} {...props} />;
...
export const VoiceSelectorDescription = ({ className, ...props }: VoiceSelectorDescriptionProps) => <span className={cn('text-xs text-muted-foreground', className)} {...props} />;
...
export const VoiceSelectorPreview = ({ className, playing, loading, onPlay, onClick, ...props }: VoiceSelectorPreviewProps) => {
    ...
    return (
        <Button aria-label={playing ? 'Pause preview' : 'Play preview'} className={cn('size-6', className)} disabled={loading} onClick={handleClick} size="icon-sm" type="button" variant="outline" {...props}>
            {icon}
        </Button>
    );
};
