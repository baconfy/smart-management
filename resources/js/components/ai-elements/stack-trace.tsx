...
import { createContext, memo, useCallback, useContext, useEffect, useMemo, useRef, useState } from 'react'; // Regex patterns for parsing stack traces
...
export const StackTraceHeader = memo(({ className, children, ...props }: StackTraceHeaderProps) => {
    ...
    return (
        <Collapsible onOpenChange={setIsOpen} open={isOpen}>
            <CollapsibleTrigger {...props} render={<div className={cn('flex w-full cursor-pointer items-center gap-3 p-3 text-left transition-colors hover:bg-muted/50', className)} />}>
                {children}
            </CollapsibleTrigger>
        </Collapsible>
    );
...
export const StackTraceContent = memo(({ className, maxHeight = 400, children, ...props }: StackTraceContentProps) => {
    ...
    return (
        <Collapsible open={isOpen}>
            <CollapsibleContent className={cn('overflow-auto border-t bg-muted/30', 'data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:animate-in data-[state=open]:fade-in-0', className)} style={{ maxHeight }} {...props}>
                {children}
            </CollapsibleContent>
        </Collapsible>
    );
...
const FilePathButton = memo(({ frame, onFilePathClick }: FilePathButtonProps) => {
    ...
    return (
        <button className={cn('underline decoration-dotted hover:text-primary', onFilePathClick && 'cursor-pointer')} disabled={!onFilePathClick} onClick={handleClick} type="button">
            {frame.filePath}
