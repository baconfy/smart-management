        ...
        <div className={cn('flex items-center gap-3', className)} {...props}>
            {children ?? (
                <>
                    ...
                    {summary.skipped > 0 && (
                        <Badge className="gap-1 bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400" variant="secondary">
...
export const TestSuiteName = ({ className, children, ...props }: TestSuiteNameProps) => {
    ...
    return (
        <CollapsibleTrigger className={cn('group flex w-full items-center gap-2 px-4 py-3 text-left transition-colors hover:bg-muted/50', className)} {...props}>
...
export const TestSuiteStats = ({ passed = 0, failed = 0, skipped = 0, className, children, ...props }: TestSuiteStatsProps) => (
...
const TestStatusIcon = ({ status }: { status: TestStatus }) => <span className={cn('shrink-0', statusStyles[status])}>{statusIcons[status]}</span>;
...
export const TestErrorStack = ({ className, children, ...props }: TestErrorStackProps) => (
    <pre className={cn('mt-2 overflow-auto font-mono text-xs text-red-600 dark:text-red-400', className)} {...props}>
        {children}
    </pre>
);
