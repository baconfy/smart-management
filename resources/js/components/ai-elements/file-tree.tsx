        ...
        <FileTreeFolderContext.Provider value={folderContextValue}>
            <Collapsible onOpenChange={handleOpenChange} open={isExpanded}>
                <div className={cn('', className)} role="treeitem" tabIndex={0} {...props}>
                    <CollapsibleTrigger render={<button className={cn('flex w-full items-center gap-1 rounded px-2 py-1 text-left transition-colors hover:bg-muted/50', isSelected && 'bg-muted')} onClick={handleSelect} type="button" />}>
                        <ChevronRightIcon className={cn('size-4 shrink-0 text-muted-foreground transition-transform', isExpanded && 'rotate-90')} />
                        <FileTreeIcon>{isExpanded ? <FolderOpenIcon className="size-4 text-blue-500" /> : <FolderIcon className="size-4 text-blue-500" />}</FileTreeIcon>
...
export const FileTreeActions = ({ className, children, ...props }: FileTreeActionsProps) => (
    ...
    <div className={cn('ml-auto flex items-center gap-1', className)} onClick={stopPropagation} onKeyDown={stopPropagation} role="group" {...props}>
        {children}
    </div>
);
